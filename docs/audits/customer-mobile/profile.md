...
4. **[RESOLVED] The "Terms of service" row previously linked to a page confirmed not to exist.** Re-verified on 2026-09-13: `apps/customer-web/src/app/(marketing)/terms/page.tsx` now exists (fixed elsewhere/earlier than this audit series, confirmed by direct inspection), so `termsUrl`'s `${websiteUrl}/terms` fallback now resolves correctly. Same root cause as `docs/audits/customer-mobile/signup.md` Finding #1, now resolved there too. "Privacy policy" on this screen was always fine — `/privacy` is a real customer-web page.

5. **[FIXED] A failed pull-to-refresh hid the entire profile screen, not just a banner.** Same cross-cutting pattern documented in `docs/audits/customer-mobile/subscriptions.md`, Finding #3: the content gate was `!loading && !error`, so a `load()` failure after a successful initial load (e.g. pull-to-refresh hiccup) unmounted the whole screen — hero card, addresses, favorites, help/account rows, everything — leaving just the `DataLoadState` error banner. A customer who already saw their profile would lose sight of it entirely on a transient network blip.
   **Fix:** gate changed to `!loading && (profile !== null || !error)` — content now stays visible once a profile has successfully loaded at least once, with the error banner shown above it on a later failed refresh instead of replacing it.

## Unused/dead fields
None beyond what's covered in Finding #1 (not a dead field so much as a field carrying data no other client can read).

## Loading/error/realtime behavior
Uses `DataLoadState` with a retry button, correctly gated on `loading && !refreshing`. No polling or realtime subscription; pull-to-refresh correctly scoped with its own `refreshing` boolean. See Finding #5 for the failed-refresh content-hiding fix.
