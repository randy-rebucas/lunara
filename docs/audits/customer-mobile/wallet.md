...
3. **[FIXED, revises prior reasoning] A failed refresh discarded the wallet balance card, top-up card, and transaction list entirely — worse, `load()`'s `catch` actively cleared `transactions` to `[]` on any failure, not just hid them.** This is the same cross-cutting pattern as `docs/audits/customer-mobile/subscriptions.md` Finding #3, present on every one of this screen's four `!loading && !error` render gates plus the `FlatList`'s `data={loading || error ? [] : transactions}` prop. The "Loading/error/realtime behavior" note below previously defended the stricter blank-on-error behavior as "reasonable" — re-examining it against the sibling bug found on the rewards/subscriptions screens, it isn't: a customer who already saw their balance and top-up history loses all of it on a single transient refresh failure (e.g. right after `topUp()`'s own `AppState`-triggered `load()` call, per Finding #1, if that particular sync's follow-up fetch hiccups), with no indication anything they'd already seen still exists.
   **Fix:** added a `loaded` flag (set after a successful load), changed all four `!loading && !error` gates to `!loading && (loaded || !error)`, changed the `FlatList` `data` prop to `loading || (error && !loaded) ? [] : transactions`, and removed the `setTransactions([])` call from `load()`'s `catch` block so a failed refresh no longer discards previously-fetched transactions even before touching the render layer.
...
Uses `DataLoadState` with a retry button. No polling; realtime balance updates only happen via the new foreground-sync (Finding #1), manual pull-to-refresh, or the next full screen load. See Finding #3 — the previous "blank on error" behavior here has been corrected to match the "preserve stale data, show the error banner above it" pattern used elsewhere in this series.
