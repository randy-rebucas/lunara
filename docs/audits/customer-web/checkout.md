...
2. **[FIXED] Same double-intent race existed in `createWalletTopupIntent`** (`payments.service.ts`, backing the customer-web Wallet page, separately audited in [wallet.md](wallet.md)). It had the identical `deleteMany`-then-`create` shape for `purpose:'wallet_topup'` payments, with no unique-index protection at the time this doc was first written.
   **Fix:** added a second partial unique index, `{userId, purpose, status}` unique where `status:'pending' AND purpose:'wallet_topup'` (`apps/api/src/modules/payments/schemas/payment.schema.ts`), scoped separately from Finding #1's `orderId`-based index so the two never collide. Wrapped `createWalletTopupIntent`'s `create()` call in the same try/catch → `isDuplicateKeyError` → `BadRequestException` pattern as `createIntent`. Typechecked `apps/api` clean.

3. **[authz] [FIXED] Payment endpoints relied solely on manual ownership checks, not route-level role guards.** `GET /payments/orders/:orderId`, `POST /payments/:id/sync`, `POST /payments/intent`, and `GET /payments/:id` (`payments.controller.ts`) only carried `JwtAuthGuard` — no `@Roles(UserRole.CUSTOMER)`. Every one of them correctly checked `order.customerId`/`payment.userId` against `req.user.sub` inside the service, so there was no live vulnerability, but it was inconsistent with `DELETE /orders/:id`'s pattern (both a `@Roles` decorator *and* a service-level check) in the same payment/order flow, and left no defense-in-depth if a future edit to any of these four service methods dropped the inline check.
   **Fix:** confirmed via repo-wide grep that no other app (admin-web, partner-web, any mobile app) calls any `/payments/*` route — only customer-web and customer-mobile do, both as the authenticated customer. Added `@UseGuards(JwtAuthGuard, RolesGuard)` + `@Roles(UserRole.CUSTOMER)` to all four routes (`payments.controller.ts`), matching the existing `orders.controller.ts` pattern. Left the mock PayMongo routes (`mock/paymongo/*`, `mock/gcash`, `mock/maya`, `mock/stripe`) and the webhook routes (`webhooks/paymongo`, `:id/confirm`) untouched — they're either dev-only (guarded by `assertMockPaymentsAllowed`'s production check) or authenticated via `PaymentWebhookGuard`, not JWT/role-based, so a customer `@Roles` guard doesn't apply to them. Typechecked `apps/api` clean; no test suite exists for this module to run.
