...
2. **[FIXED] Two concurrent top-up requests could both create a pending payment — double PayMongo checkout session, or a double wallet debit if both were somehow completed.** Flagged during the checkout module audit ([checkout.md](checkout.md), Finding #2) as a race structurally identical to the one fixed there: `createWalletTopupIntent`'s `deleteMany`-then-`create` wasn't atomic, so two near-simultaneous requests (double-click, two tabs) could both pass every check and both create a `PENDING` `wallet_topup` payment row.
   **Fix:** added a partial unique index `{userId, purpose, status}` (unique where `status:'pending' AND purpose:'wallet_topup'`) in `apps/api/src/modules/payments/schemas/payment.schema.ts`, scoped separately from the order-payment index so the two never collide. `createWalletTopupIntent`'s `create()` call now catches the resulting E11000 via the existing `isDuplicateKeyError` helper and returns a clean `BadRequestException` instead of proceeding to PayMongo checkout. See [checkout.md](checkout.md) Finding #2 for full detail (fix landed in the same `payments.service.ts`/`payment.schema.ts` change).
