...
import { BubbleField, DARK_PANEL_BUBBLES, LIGHT_PANEL_BUBBLES } from './bubble-field';
import { clearPartnerToken, getMyBranding, getPartnerToken, staffLogin } from '../lib/partner-api';
...
/** Shared login UI for both the tenant-agnostic `/login` (no slug known yet — a fresh session,
 * a 401 redirect, a logout) and the `/{partnerSlug}/login` deep link (slug already known, e.g. a
 * bookmarked or shared portal URL). `defaultNext` is where a successful sign-in lands when the
 * URL carries no explicit `?next=`. */
export function PortalLoginScreen({
  defaultNext,
  expectedSlug,
}: {
  defaultNext: string;
  /** When set (the /{partnerSlug}/login deep link), the signed-in account's own shop must match
   * this slug — otherwise the credentials are valid but for a different shop, and we should say
   * so rather than silently redirecting into that other shop's portal. */
  expectedSlug?: string;
}) {
    ...
    getMyBranding()
      .then(({ slug }) => {
        ...
        if (slug && slug !== expectedSlug) {
          clearPartnerToken();
          setError("You're signed in to a different shop. Sign out there first, or sign in with this shop's own credentials.");
          return;
        }
        router.replace(nextPath);
      })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (expectedSlug) {
        ...
        if (slug && slug !== expectedSlug) {
          clearPartnerToken();
          setError("These credentials belong to a different shop. Sign in from your own shop's portal.");
          return;
        }
      }
      router.replace(nextPath);
