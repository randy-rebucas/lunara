...
/** Mirrors RiderMessagingController for the customer surface — same conversation/message
 * collections, keyed by the customer's own user id, single admin-support channel only (customers
 * have no employer/workforce channel equivalent). */
...
export class CustomerMessagingController {
  ...
  async getConversation(@Req() req: any) {
    ...
    const data = await this.messaging.getOrCreateConversation(customerId);
