...
import { PickupOfferCard, DeliveryOfferCard, RouteRow } from '../../src/components/rider-offer-card';
    ...
    if (row.kind === 'pickup_offer') {
      return (
        <PickupOfferCard
          ...
          onDecline={() => {
            setDismissedPickup((s) => new Set([...s, row.item._id]));
            declinePickupOffer(row.item._id);
          }}
        />
      );
    }
    ...
    if (row.kind === 'delivery_offer') {
      return (
        <DeliveryOfferCard
          ...
          onDecline={() => {
            setDismissedDelivery((s) => new Set([...s, row.item._id]));
            declineDeliveryOffer(row.item._id);
          }}
        />
      );
    }
