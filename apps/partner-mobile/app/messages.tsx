...
export default function MessagesScreen() {
  ...
  const loadMessages = useCallback(async (convId: string) => {
    ...
    await partnerFetch(`/partner/messages/${convId}/read`, { method: 'PATCH' }).catch(() => {});
  }, []);
  ...
  usePartnerMessagingSocket({
    ...
    onNewMessage: (msg) => {
      ...
      if (conversation) {
        void partnerFetch(`/partner/messages/${conversation._id}/read`, { method: 'PATCH' }).catch(() => {});
      }
    },
  });
  ...
  async function uploadAttachment(conversationId: string, file: UploadFile): Promise<MessageAttachment> {
    return partnerUpload<MessageAttachment>(`/partner/messages/${conversationId}/upload`, file);
  }
